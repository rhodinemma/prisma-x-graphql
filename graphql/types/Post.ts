    ...
    definition(t) {
        ...
        t.nullable.field('author', {
            ...
            async resolve(_parent, _args, ctx) {
                return await ctx.prisma.Post.findUnique({ where: { id: _parent.id } }).author();
            }
        })
    }
});
    ...
    definition(t) {
        t.field('post', {
            ...
            async resolve(_, args, ctx) {
                return await ctx.prisma.post.findUnique({ where: { id: Number(args.postId) } })
            }
        });
        ...
        t.list.field('feed', {
            type: 'Post',
            async resolve(_parent, _args, ctx) {
                return await ctx.prisma.post.findMany({ where: { published: true } })
            }
        });
        ...
        t.list.field('drafts', {
            type: 'Post',
            async resolve(_parent, _args, ctx) {
                return await ctx.prisma.post.findMany({ where: { published: false } })
            }
        });
